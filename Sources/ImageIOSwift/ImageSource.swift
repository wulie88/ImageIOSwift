...
public class ImageSource {
	...
	public func cgImage(at index: Int = 0, options: ImageOptions? = nil) -> CGImage? {
		return CGImageSourceCreateImageAtIndex(self.cgImageSource, index, options?.rawValue)
	}
	...
	public func cgThumbnailImage(at index: Int = 0, options: ImageOptions? = nil) -> CGImage? {
		return CGImageSourceCreateThumbnailAtIndex(self.cgImageSource, index, options?.rawValue)
	}
	...
	public var status: CGImageSourceStatus {
		return CGImageSourceGetStatus(self.cgImageSource)
	}
	...
	public func status(at index: Int) -> CGImageSourceStatus {
		return CGImageSourceGetStatusAtIndex(self.cgImageSource, index)
	}
	
	public func error(at index: Int) -> Error? {
		return Error(self.status(at: index))
	}
